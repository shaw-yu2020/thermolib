...
/// PC-SAFT EOS :: PcSaftMix2
/// ```
/// use thermolib::PcSaftMix2;
/// let mut fluids = PcSaftMix2::new_fluid(
///     [0.5, 0.5],
///     [1.0000, 1.6069],
///     [3.7039, 3.5206],
///     [150.03, 191.42],
...
/// let py = fluids.tx_flash(198.15, 0.5).unwrap();
/// assert_eq!((py.1 * 1e5).round() / 1e5, 0.92057);
/// let px = fluids.ty_flash(198.15, 0.5).unwrap();
/// assert_eq!((px.1 * 1e5).round() / 1e5, 0.04226);
/// let mut fluids = PcSaftMix2::new_fluid(
///     [0.5, 0.5],
///     [2.0729, 1.5255],
///     [2.7852, 3.2300],
...
///     0.0,
/// ); // CO2+CH3OH_2B
/// fluids.set_2B_assoc_term(1, 0.035176, 2899.5);
/// fluids.tp_flash(298.15, 0.1e6).unwrap();
...
///     0.0,
/// ); // CL2_QQ+C6H6_QQ
/// fluids.set_polar_term([3.0724, 5.5907], [4, 4]);
/// fluids.tp_flash(298.15, 0.1e6).unwrap();
/// assert_eq!(fluids.rho().unwrap().round(), 14137.0);
/// let py = fluids.tx_flash(298.15, 0.5).unwrap();
/// assert_eq!((py.1 * 1e5).round() / 1e5, 0.97915);
/// let px = fluids.ty_flash(298.15, 0.5).unwrap();
/// assert_eq!((px.1 * 1e5).round() / 1e5, 0.02098);
/// let mut fluids = PcSaftMix2::new_fluid(
///     [0.5, 0.5],
///     [2.9835, 2.6001],
///     [3.4239, 3.2872],
...
/// fluids.set_polar_term([2.78, 2.72], [2, 2]);
/// fluids.tp_flash(298.15, 0.1e6).unwrap();
/// assert_eq!(fluids.rho().unwrap().round(), 12165.0);
/// let py = fluids.tx_flash(298.15, 0.5).unwrap();
/// assert_eq!((py.1 * 1e5).round() / 1e5, 0.22132);
/// let px = fluids.ty_flash(298.15, 0.5).unwrap();
/// assert_eq!((px.1 * 1e5).round() / 1e5, 0.78066);
/// let mut fluids = PcSaftMix2::new_fluid(
///     [0.5, 0.5],
///     [1.5131, 2.7447],
///     [3.1869, 3.2742],
...
impl PcSaftMix2 {
    ...
    pub fn set_1_assoc_term(&mut self, n: usize, kappa_AB: f64, epsilon_AB: f64) {
