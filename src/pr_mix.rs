use crate::algorithms::shengjin_roots;
use std::f64::consts::SQRT_2;
const SQRT2_ADD_POS1: f64 = SQRT_2 + 1.0;
const SQRT2_ADD_NEG1: f64 = SQRT_2 - 1.0;
const R_CONST: f64 = 8.314462618;
const AC_COEF: f64 = 0.45724;
const BC_COEF: f64 = 0.07780;
pub struct PrMix {
    params: Vec<PrParams>,
}
impl PrMix {
    pub fn new_fluid(crit_t: &[f64], crit_p: &[f64], omega: &[f64]) -> Self {
        let mut params = Vec::with_capacity(omega.len());
        if omega.len() == crit_t.len() && omega.len() == crit_p.len() {
            for i in 0..omega.len() {
                params.push(PrParams {
                    ac: AC_COEF / crit_p[i] * (R_CONST * crit_t[i]).powi(2),
                    bc: BC_COEF / crit_p[i] * R_CONST * crit_t[i],
                    kappa: 0.37464 + 1.54226 * omega[i] - 0.26992 * omega[i].powi(2),
                    crit_t: crit_t[i],
                    sqrt_a: (0.0, 0.0),
                });
            }
        }
        Self { params }
    }
    #[allow(non_snake_case)]
    pub fn calc_fi(&mut self, temp: f64, pres: f64, z: &Vec<f64>, is_liquid: bool) -> Vec<f64> {
        let sum_zaj = z
            .iter()
            .zip(&mut self.params)
            .map(|(z, j)| z * j.sqrt_a(temp))
            .sum::<f64>();
        let a = z
            .iter()
            .zip(&mut self.params)
            .map(|(z, i)| z * i.sqrt_a(temp) * sum_zaj)
            .sum::<f64>();
        let b = z
            .iter()
            .zip(&mut self.params)
            .map(|(z, i)| z * i.b())
            .sum::<f64>();
        let (A, B) = (
            a * pres / (R_CONST * temp).powi(2),
            b * pres / (R_CONST * temp),
        );
        let (Zv, Zl) = shengjin_roots(
            B - 1.0,
            A - 3.0 * B.powi(2) - 2.0 * B,
            -A * B + B.powi(2) + B.powi(3),
        );
        let Z = if is_liquid && Zl > 0.0 { Zl } else { Zv };
        self.params
            .iter()
            .zip(z)
            .map(|(i, z)| {
                z * pres
                    * (i.b() / b * (Z - 1.0)
                        - (Z - B).ln()
                        - A / (2.0 * SQRT_2 * B)
                            * (2.0 * sum_zaj / a - i.b() / b)
                            * ((Z + SQRT2_ADD_POS1 * B) / (Z - SQRT2_ADD_NEG1 * B))
                                .abs()
                                .ln())
                    .exp()
            })
            .collect()
    }
    pub fn tpz_flash(&mut self, _temp: f64, _pres: f64, _z: &[f64]) {
        println!("tpz_flash");
    }
}
struct PrParams {
    ac: f64,
    bc: f64,
    kappa: f64,
    crit_t: f64,
    sqrt_a: (f64, f64),
}
impl PrParams {
    fn sqrt_a(&mut self, temp: f64) -> f64 {
        if temp != self.sqrt_a.0 {
            self.sqrt_a = (
                temp,
                (self.ac * (1.0 + self.kappa * (1.0 - (temp / self.crit_t).sqrt())).sqrt()).sqrt(),
            )
        }
        self.sqrt_a.1
    }
    fn b(&self) -> f64 {
        self.bc
    }
}
/// unit test
#[cfg(test)]
mod tests {
    use super::*;
    #[test]
    fn test_pr_mix() {
        let n2 = (0.025, 126.19, 3.3958e6, 0.03720);
        let c1 = (0.650, 190.56, 4.5992e6, 0.01142);
        let c2 = (0.150, 305.32, 4.8722e6, 0.09950);
        let c3 = (0.150, 369.89, 4.2512e6, 0.15210);
        let c4 = (0.025, 425.13, 3.7960e6, 0.20100);
        let mut example1 = PrMix::new_fluid(
            &[n2.1, c1.1, c2.1, c3.1, c4.1],
            &[n2.2, c1.2, c2.2, c3.2, c4.2],
            &[n2.3, c1.3, c2.3, c3.3, c4.3],
        );
        example1.tpz_flash(300.0, 5.5e6, &vec![n2.0, c1.0, c2.0, c3.0, c4.0]);
    }
}
